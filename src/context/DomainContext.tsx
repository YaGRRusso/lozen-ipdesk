...
const initialData: DomainTS = {
  email_address:
    import.meta.env.VITE_EMAIL ?? localStorage.getItem("zd_email") ?? "",
  locale:
    import.meta.env.VITE_LOCALE ?? localStorage.getItem("zd_locale") ?? "pt-br",
  password:
    import.meta.env.VITE_PASSWORD ?? localStorage.getItem("zd_password") ?? "",
  subdomain:
    import.meta.env.VITE_SUBDOMAIN ??
    localStorage.getItem("zd_subdomain") ??
    "",
};
